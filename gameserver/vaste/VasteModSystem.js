...
const { log: logger, error } = require("./Logger");
...
class VasteModSystem {
  constructor(gameServer) {
    ...
    this.log = (msg) => {
      if (this.gameServer && this.gameServer.log) return this.gameServer.log(`${msg}`);
      logger(`[VASTE-MOD] ${msg}`);
    };
  ...
  _populateLuaGlobals(L, modPath) {
    ...
    setGlobal("GetPlayerEntity", (player) => {
      const id = typeof player === "object" ? player.id : player;
      return this.entityManager.getPlayerEntity(id);
    });
    setGlobal("GetPlayerName", (player) => {
      return player && player.username ? player.username : player && player.name ? player.name : String(player);
    });
    setGlobal("SetEntityInWorld", (entity, world) => {
      if (!entity || !world) return;
      this.entityManager.setEntityInWorld(entity, world);
    });
    setGlobal("SetEntityCoords", (entity, position) => {
      if (!entity || !position) return;
      this.entityManager.setEntityCoords(entity, position);
      if (entity && entity.type === "player" && this.gameServer) this.gameServer.updatePlayerPosition(entity.playerId, position.x, position.y, position.z);
    });
            ...
            try {
              ...
              for (const a of args) {
                try {
                  ...
                  try {
                    interop.push(state, String(a));
                  } catch (e2) {
                    lua.lua_pushstring(state, to_luastring(String(a)));
                  }
                }
              }
              ...
              if (status !== 0) {
                ...
                lua.lua_pop(state, 1);
                error(`[mod][lua callback] error calling listener for '${evName}': ${err}`);
              }
            } catch (e) {
              error(`[mod][lua callback] exception invoking Lua listener for '${evName}': ${e.message}`);
            }
          };
  ...
  async loadMods() {
    ...
    this.log(`Successfully loaded ${this.loadedMods.size} mod(s)`);
  }
    ...
    for (const serverScriptRel of modContext.serverScripts) {
      ...
      try {
        ...
        if (callStatus !== 0) {
          ...
          let luaTypeNum = -1;
          try {
            luaTypeNum = lua.lua_type(L, -1);
            luaTypeName = lua.lua_typename(L, luaTypeNum);
          } catch (e) {
            /* ignore */
          }
          ...
          try {
            if (interop && typeof interop.tojs === "function") {
              ...
              try {
                interopRepr = require("util").inspect(jsVal, { depth: 2 });
              } catch (e) {
                interopRepr = String(jsVal);
              }
            }
          } catch (e) {
            interopRepr = `interop.tojs failed: ${e && e.message}`;
          }
